...
const CardThree = ({ item }) => {
  ...
  const isEqualToPrice =
    numberParser(item?.price) === numberParser(item?.original_price);
  const isNullPrice =
    numberParser(item?.price) === 0 || numberParser(item?.original_price) === 0;
  ...
  return (
    <div
      onClick={(e) => handleOpenSourceSite(e, item?.url)}
        ...
        <div className="flex flex-col gap-y-1 max-h-max">
          ...
          {!isNullPrice && (
            <div className="flex items-center flex-wrap gap-x-2 gap-y-1">
              <p>{item?.currency ?? "BDT"}{item?.price}</p>
              {!isEqualToPrice && (
