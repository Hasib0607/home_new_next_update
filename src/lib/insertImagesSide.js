      ...
      if (imageIndex !== undefined && !usedBannerIndexes.includes(imageIndex)) {
        gridItems.push(
          <div key={`image${index}`} className="pb-5">
            <a href={portrait[imageIndex]?.link} target="_blank" rel="noopener noreferrer">
              <Image
                src={portrait[imageIndex]?.banner}
                width={500}
                height={500}
                alt={`images`}
  ...
  if (data?.length < 10 && portrait[0]?.banner) {
    gridItems.push(
      <div key={`imageFallback`} className="pb-5">
        <a href={portrait[0]?.link} target="_blank" rel="noopener noreferrer">
          <Image
            src={portrait[0]?.banner}
            width={500}
            height={500}
            alt={``}
            className="min-w-full h-auto"
