...
export const fetchTypeWiseBlogData = async (blogTypeId, typePage) => {
  try {
    const response = await fetch(`${baseUrl}/blog/types/${blogTypeId}${typePage}`, {
      next: { revalidate: 10 },
    });
    const data = await response.json();
    return data;
...
export const fetchBlogDetailsData = async (params) => {
  try {
    const response = await fetch(`${baseUrl}/blog/details/${params?.slug}`, {
      next: { revalidate: 10 },
    });
    const data = await response.json();
    return data;
...
export const fetchPseCategory = async () => {
  try {
    const response = await fetch(`${baseUrl}/pse/products/categories`, {
      next: { revalidate: 10 },
    });
    const data = await response.json();
    return data?.results;
...
export const fetchPseSearch = async (searchTxt, categorySlug) => {
  try {
    const response = await fetch(
      `${baseUrl}/pse/products/product-by-category?name=${searchTxt}&slug=${categorySlug}`
    );
    const data = await response.json();
    return data;
  } catch (error) {
